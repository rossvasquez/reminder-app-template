...
export default function SignUp({ChangeState}) {
    ...
    return(
        <form onSubmit={(e) => preventDefault(e)} className='flex flex-col justify-center items-center w-full max-w-3xl px-6 py-6 bg-zinc-800 rounded-lg shadow-lg'>
            ...
            <input required onChange={(e) => handleEmail(e)} value={SignUpInfo.email} type='email' name='email' className="mt-1 focus:outline-none w-full text-2xl rounded-lg h-14 font-light text-zinc-800 px-3" />
            <label htmlFor="password" className="w-full text-left text-white font-light text-xl pl-2 mt-4">Password</label>
            <input required onBlur={() => checkPasswordMatch()} onChange={(e) => handlePassword(e)} value={SignUpInfo.password} type='password' name='password' className="mt-1 focus:outline-none w-full text-2xl rounded-lg h-14 font-light text-zinc-800 px-3" />
            <label htmlFor="passwordCheck" className="w-full text-left text-white font-light text-xl pl-2 mt-4">Confirm Password</label>
            <input required onBlur={() => checkPasswordMatch()} onChange={(e) => handlePassword2(e)} value={SignUpInfo.password2} type='password' name='passwordCheck' className="mt-1 focus:outline-none w-full text-2xl rounded-lg h-14 font-light text-zinc-800 px-3" />
            {PasswordMatch ? null : <p className="text-xl text-cyan-400 mt-4 w-full font-light">Passwords Do Not Match</p>}
            <button type="submit" className="w-full h-16 rounded-lg bg-cyan-400 text-white text-2xl font-light hover:cursor-pointer mt-6">Sign Up</button>
        </form>
    )
}