...
export default function Landing({GoToSignUp, GoToLogIn}) {
    ...
    return(
        <div className='flex flex-col justify-center items-center px-6 md:px-24 py-8 md:py-16 bg-zinc-800 rounded-lg shadow-lg'>
          ...
          <div className="flex flex-col-reverse md:flex-row gap-4 mt-16 max-w-[36rem] w-full">
            <button onClick={GoToSignUp} className="h-16 md:h-20 w-full bg-transparent border-2 border-white text-white rounded-full text-3xl pb-1 font-light hover:cursor-pointer hover:text-cyan-400">Sign Up</button>
            <button onClick={GoToLogIn} className="h-16 md:h-20 w-full bg-cyan-400 text-zinc-800 rounded-full text-3xl pb-1 font-light hover:shadow-md hover:cursor-pointer hover:text-white">Log In</button>
          </div>
        </div>
    )
}